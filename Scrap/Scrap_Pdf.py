...
def download_pdf(pdf_url, save_path):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Échec ({response.status_code}) : {pdf_url}")
            return False
...
def extract_images(pdf_path):
    ...
    for page_num, page in enumerate(doc):
        ...
        for img_index, img in enumerate(page.get_images(full=True)):
            ...
            # ✅ Vérification si la légende est bien extraite
            print(f"Légende trouvée pour l'image {img_index} page {page_num}: {caption}")

            images_data.append({
                "page": page_num + 1,
                "image_data": image_base64,
                "caption": caption if caption else "Légende non disponible"
            })

    return images_data

def find_nearest_caption(text, img_index):
    """Détection améliorée des légendes autour de l'image"""
...
def process_pdf(index, row):
    ...
    document = {
        ...
        "tables": extracted_tables,  # ✅ Correction : stocker les tableaux sous "tables"
        "images": extracted_images
    }

    collection.insert_one(document)
    print(f"✅ Données insérées pour {pdf_name} 🚀")

# 📌 Traitement des PDFs en parallèle
